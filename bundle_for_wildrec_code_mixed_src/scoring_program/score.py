...
if __name__ == "__main__":
    ...
    sys.stdout.write(f'input dir {os.listdir(input_dir)}')
    ...
    sys.stdout.write(f"ref: {os.listdir(os.path.join(input_dir, 'ref'))}")
    sys.stdout.write(f"res: {os.listdir(os.path.join(input_dir, 'res'))}")

    ref_file_name = os.listdir(os.path.join(input_dir, 'ref'))[0]
    solution_file = os.path.join(input_dir, 'ref', ref_file_name)
    ...
    # solution_names = sorted(ls(os.path.join(input_dir, 'ref', 'gold_test.csv')))
    solution_df = pd.read_csv(solution_file)
    prediction_df = pd.read_csv(predict_file)
    if (solution_df.shape[0] != prediction_df.shape[0]): raise ValueError(
            "Bad prediction shape. Prediction shape: {}\nSolution shape:{}".format(prediction_df.shape[0], solution_df.shape[0]))
    
    prediction_df['sentiment'] = prediction_df['sentiment'].str.lower()
    solution_df['sentiment'] = solution_df['sentiment'].str.lower()
    fin_x = prediction_df.merge(solution_df, on='ids', how='inner', suffixes=['_pred', '_gold'])
    sent_gold = fin_x['sentiment_gold'].values.tolist()
    sent_pred =  fin_x['sentiment_pred'].values.tolist()
    print('=====================Data loaded successfully======================')

    try:
        sys.stdout.write(f"{sent_gold[0]}, {sent_pred[0]}")
        sys.stdout.write(f"{len(sent_gold)}, {len(sent_pred)}")
        precision, recall, f1 = scoring_function(sent_gold, sent_pred)
    ...
    score_file.write("f1_macro: %0.12f\n" % f1)
    score_file.write("precision_macro: %0.12f\n" % precision)
    score_file.write("recall_macro: %0.12f\n" % recall)
    ...
    #     try:
    #         ## changes to be made in order to read the input, output files as 
    ...
    #         # this has to be modified or written three times with all scores
